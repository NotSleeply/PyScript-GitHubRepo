"""Per-repo materialization: ZIP download or Git clone/pull. Uses lazy
imports for `requests`, `git`, `tenacity` so callers that never actually
download (agents doing dry-runs, --help, etc.) don't pay the cost."""
...
logger = get_logger("github.downloader")


class RetryableError(Exception):
    """Raised when the operation failed for a transient reason (5xx,
    network blip) and retry is appropriate."""


class NonRetryableError(Exception):
    """Raised when retry cannot help (branch doesn't exist, permission
    denied, etc.). Caller should record the failure and move on."""


def _retry_on_retryable(fn):
    """Lazy-wrap `fn` with tenacity retry on first call so importing this
    module does not load tenacity."""
    wrapped = None
...
def download_zip(repo, opts, progress, task_id):
    ...
    repo_name = repo['name']
    target_ref = opts.get('target_ref') or repo.get('default_branch', 'main')
    ...
    try:
        ...
        if not resp:
            raise NonRetryableError(
                f"Branch or Tag '{target_ref}' (and fallback '{fallback_ref}') not found for {repo_name}."
            )
    ...
    try:
        if os.path.exists(os.path.join(repo_path, '.git')):
            ...
            try:
                ...
                msg = str(e).lower()
                if (
                    ('not found' in msg or 'could not read from remote' in msg or 'remote branch' in msg)
                    and target_ref != fallback_ref
                ):
